<!-- Main Content Area: Form Pengajuan Alat -->
<main class="flex-1 flex flex-col h-full rounded-3xl bg-white/30 backdrop-blur-xl border border-white/50 shadow-xl overflow-y-auto">
    ...
    <header class="p-6 border-b border-white/30 bg-white/20 flex items-center gap-4">
        ...
        <div>
            <h3 class="text-2xl font-semibold text-slate-800 tracking-tight">Buat Pengajuan Baru</h3>
            <p class="text-sm text-slate-600 mt-1">Isi detail alat kesehatan yang memerlukan kalibrasi atau pengecekan.</p>
        </div>
    </header>
    ...
    <div class="p-6 flex-1 flex justify-center items-start pt-10">
        ...
        <div class="w-full max-w-2xl p-8 rounded-3xl bg-white/50 backdrop-blur-md border border-white/60 shadow-sm">
            ...
            <form action="<?= site_url('request/store') ?>" method="POST" class="space-y-6">
                <!-- CSRF Token -->
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="equipment_name" required
                            class="w-full px-4 py-3 rounded-xl bg-white/60 border border-white/60 focus:bg-white/90 focus:ring-2 focus:ring-indigo-400 focus:outline-none transition-all placeholder-slate-400 text-slate-800 shadow-inner"
                            placeholder="Contoh: Patient Monitor Mindray uMEC10">
                        <p class="text-xs text-slate-500 mt-2">Masukkan merek dan tipe spesifik dari alat kesehatan.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="serial_number" required
                            class="w-full px-4 py-3 rounded-xl bg-white/60 border border-white/60 focus:bg-white/90 focus:ring-2 focus:ring-indigo-400 focus:outline-none transition-all placeholder-slate-400 text-slate-800 shadow-inner font-mono"
                            placeholder="Contoh: SN-88293012">
                    </div>
                </div>
                ...
                <div class="pt-4 flex justify-end gap-3 border-t border-white/40">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
